...
class CQL(OffPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if isinstance(self.alpha_coef, str) and self.ent_coef.startswith("auto"):
            ...
            # this will throw an error if a malformed string (different from 'auto')
            # is passed
            log_alpha_coef_def = LogAlphaCoef(self.ent_coef)
            self.key, temp_key = jax.random.split(self.key, 2)
            self.log_alpha_coef = Model.create(log_alpha_coef_def, inputs=[temp_key])
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            self.key, new_actor, new_critic, new_critic_target, new_log_ent_coef, new_log_alpha_coef, info \
                = _update_jit(key, self.actor, self.critic, self.critic_target, self.log_ent_coef, self.log_alpha_coef, replay_data,
                              self.gamma, self.target_entropy, self.tau, target_update_cond, self.entropy_update, self.alpha_update,
                              self.conservative_weight, self.lagrange_thresh)
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(CQL, self)._excluded_save_params() + ["actor", "critic", "critic_target", "log_ent_coef", "log_alpha_coef"]
